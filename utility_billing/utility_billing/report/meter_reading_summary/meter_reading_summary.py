...
def get_data(filters):
    ...
    # Pull all parent Meter Reading docs
    ...
    for reading in readings:
        # For each Meter Reading, get its items
        ...
        for item in items:
            # For each item, get tariff rates (block/qty/rate/amount)
            ...
            if rates:
                ...
                # Item row with no tariff breakdown
                data.append(
                    {
...
def get_chart_data(data):
    ...
    for row in data:
        if row.get("type") in ("Item", "Rate"):
            ...
            consumption_data[item_code] = consumption_data.get(item_code, 0) + (
                row.get("consumption") or 0
            )
...
def get_report_summary(data):
    ...
    total_readings = len(
        set(row.get("name") for row in data if row.get("name"))
    )
    total_consumption = sum(
        row.get("consumption", 0) for row in data if row.get("consumption")
    )
    total_amount = sum(
        row.get("amount", 0) for row in data if row.get("amount")
    )
    currency = next(
        (
            row.get("currency")
